...
urlpatterns = [
    ...
    url(r'^js/question/(?P<pk>\d+)/like/$',
        login_required(VotesView.as_view(model=Question, vote_type=Like.LIKE)), name='question_like'),
    ...
    url(r'^js/answer/(?P<pk>\d+)/like/$',
        login_required(VotesView.as_view(model=Answer, vote_type=Like.LIKE)), name='answer_like'),
